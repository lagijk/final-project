...
import { Match, Player, MatchType, PlayerType, Profile } from "@/type";
import ProfileCard from "@/components/ProfileCard";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import {
    Box, Typography, Card, Stack, Chip, TextField, Button,
    Accordion, AccordionSummary, AccordionDetails, Avatar, Grid
} from "@mui/material";
    ...
    useEffect(() => {
        ...
        async function fetchMatchHistory() {
            try {
                ...
                // we need to use encodeURIComponent because user will enter # for tag, and we need to convert that to % for backend
                ...
                setProfile(summonerResult); //  save for ProfileCard rendering
                ...
                const matchInfo = await Promise.all(
                    matchResults.slice(0, 20).map((matchResult: string) =>
                ...
                const matchDetail: Match[] = matchInfo.map((match: MatchType) => {
                    ...
                    const winStatus = match.info.teams.find((team) => team.teamId === user?.teamId)?.win ?? false;
            ...
            {profile && (
                <Box mb={4}>
                    <ProfileCard
                        ...
                        mainChampionName={match[0]?.player?.championName || "Ahri"}
                ...
                <Accordion key={i}>
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                        <Card
                            sx={{
                                width: "100%",
                                backgroundColor: m.win ? "#e3f2fd" : "#ffebee",
                                display: "flex",
                                alignItems: "center",
                        ...
                        >
                            <Stack direction="row" spacing={2} alignItems="center" sx={{ ml: 1 }}>
                                <Avatar
                                    src={`https://ddragon.leagueoflegends.com/cdn/15.8.1/img/champion/${m.player?.championName}.png`}
                                    alt={m.player?.championName}
                                        ...
                                        {m.participants.slice(teamIndex * 5, teamIndex * 5 + 5).map((p, index) => (
                                            ...
                                            >
                                                <Stack direction="row" spacing={2} alignItems="center">
                                                    <Avatar
                                                        src={`https://ddragon.leagueoflegends.com/cdn/15.8.1/img/champion/${p.championName}.png`}
                                                        alt={p.championName}
